...
function toTable(json) {
    ...
    json[0].forEach(entry => {
        ...
        syntax += '<tr><td>'+ (entry.client_company == null?'':entry.client_company) +'</td><td>'+ entry.last_name + ', ' + entry.first_name +'</td><td>' + (entry.job_title == null?'':entry.job_title) + '</td><td>'+ (entry.address1 == null?'':entry.address1) + '<br>' + (entry.city == null?'':entry.city) + ', ' + entry.state + ' ' + (entry.zip_code == null?'':entry.zip_code) + '</td><td>'+ (entry.work_phone == null?'':entry.work_phone) +'</td><td>'+ (((entry.email != null || entry.email != undefined) && entry.email.includes('@'))?'<a href="mailto:'+entry.email+'">'+entry.email+'</a>':'') +'</td><td><button type="button" id="'+ entry.ID + '" onclick="edit('+ entry.ID +');">Edit</button></td></tr>';
