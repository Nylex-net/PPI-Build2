...
connection.query('SELECT ID, Active, First, Last, Email, PM FROM Contacts').then(data => {
    data.forEach((element) => {
        var query = "INSERT INTO master.dbo.Staff VALUES ("+element.ID+", "+((element.Active == 'Yes')?1:0)+", '"+element.First.replace(/'/gi, "''")+"', '"+element.Last.replace(/'/gi, "''")+"', '"+element.Email+"', "+(element.PM==-1?1:0)+", 0)";
...
(async () => {
    //const conn = await pool.acquire();
        ...
        // ...
    pool.open();
    console.log("Adding Projects & Billing Groups");
    projects(); // And Billing Groups.
    ...
    connection.query("SELECT * FROM Projects WHERE Projectid IS NOT NULL AND Projectid <> '' AND ProjectTitle IS NOT NULL AND ProjectTitle <> ''").then(data => {
        ...
        data.forEach((element) => {
            ...
            if(element.BillGrp == null || element.BillGrp == 'NULL' || (typeof element.BillGrp == 'string' && element.BillGrp.trim() == '')) {
                ...
                "job_title, address1, address2, city, state, zip_code, work_phone, ext, home_phone, cell, fax, email, binder_size, binder_location, description_service) "+
                "VALUES ('"+// element.Id +", '"+
                element.Projectid+"', '" +
                        ...
                        if(error) {
                            ...
                            pool.query(query, (erro, sussybaka) => {
                                if(erro) {
                                    ...
                                    pool.query("SELECT TOP 1 ID FROM BillingGroups WHERE project_ID = '" + row[0].ID + "' AND group_number = '" + groupNumber + "'", (err, poopID) => {
                                        if(err) {
...
function promos() {
    connection.query("SELECT * FROM Projects WHERE ProjectTitle IS NOT NULL AND ProjectTitle <> '' AND PromoId IS NOT NULL AND PromoId <> ''").then(data => {
        ...
        data.forEach((element) => {
            ...
            var query = "IF NOT EXISTS (SELECT 1 FROM Promos WHERE promo_id = '"+element.PromoId+"') BEGIN INSERT INTO Promos "+
            ...
            "job_title, address1, address2, city, state, zip_code, work_phone, ext, home_phone, cell, fax, email, binder_size, description_service) "+
            ...
            pool.query(query, (err, rows) => {
                if(err) {
                    ...
                    pool.query("SELECT ID FROM Promos WHERE promo_id = '" + element.PromoId + "'", (err, rows) => {
                        if(err) {
                            console.log("Cannot get promo: " + element.PromoId);
                            console.error(err);
                        }
                        else {
                            if(element.TeamMembers != null && element.TeamMembers != "NULL" && element.TeamMembers != "") {
                                ...
                                if(memberArray.length > 0) {
                                    if(rows.length > 0) {
                                        memberArray.forEach((member) => {
                                            var query = "INSERT INTO PromoTeam VALUES ("+ rows[0].ID + ", " + member + ");";
                                            pool.query(query, (error) => {
                                                if(error) {
                                                    console.log("Error for Promo member: " + member)
                                                    console.log(query);
                                                    console.error(error);
                                                }
                                            });
                            ...
                            if(element.ProjectKeywords != null && element.ProjectKeywords != "NULL" && element.ProjectKeywords != "") {
                                ...
                                pool.query(query, (err, IDs) => {
                                    if(err) {
                                        ...
                                        IDs.forEach((id) => {
                                            query = "INSERT INTO PromoKeywords VALUES (" + rows[0].ID + ", " +id.ID+ ")";
                                            pool.query(query, (err) => {
                                                if(err) {
                                                    console.log("Cannot add keyword ID " + id.ID + " to Promo ID " + rows[0].ID);
                                                    console.error(err);
                                                }
                                            });
                                        });
